  ...
  const handleTelegramReminder = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert("Could not generate Telegram link. Try again.");
      }
    } catch {
      ...
      <div className="max-w-[1200px] mx-auto space-y-8">
        ...
        <div className="flex items-center justify-between gap-6 flex-wrap animate-in delay-100 relative z-50">
          ...
          <div className="flex items-center gap-3 relative">
            ...
            <button
              onClick={handleTelegramReminder}
              disabled={telegramLoading}
              className="flex items-center gap-2 px-4 py-2 rounded-lg border border-soft text-main bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700 transition cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Send size={18} className="text-blue-500" />
              <span className="hidden sm:inline">
                {telegramLoading ? "Linking..." : "Telegram Reminder"}
              </span>
            </button>
