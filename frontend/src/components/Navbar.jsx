          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center md:hidden gap-2">
              <motion.button
                whileTap={{ scale: 0.95 }}
                onClick={handleThemeToggle}
                className="p-2 rounded-xl border border-soft text-main"
              >
                {theme === "dark" ? <Sun size={18} className="text-yellow-400" /> : <Moon size={18} className="text-[#3b8ea0]" />}
              </motion.button>
              
              <button
                onClick={() => setIsOpen(!isOpen)}
                className="p-2 rounded-xl text-[#4eb7b3] hover:bg-[#d0f6e3]/50 transition-colors"
              >
