...
## Table of Contents
- [🚨 MANDATORY SESSION START — DO THIS BEFORE ANYTHING ELSE](#-mandatory-session-start-do-this-before-anything-else)
  - [Why this exists:](#why-this-exists)
  - [Hallucination Prevention Rules:](#hallucination-prevention-rules)
  - [OpenCode Delegation — MANDATORY:](#opencode-delegation-mandatory)
- [🪙 Token Optimization — Read This First](#-token-optimization-read-this-first)
  - [At the START of every session on an existing project:](#at-the-start-of-every-session-on-an-existing-project)
  - [Never read these (covered by .agentignore):](#never-read-these-covered-by-agentignore)
  - [Never re-read files already in context:](#never-re-read-files-already-in-context)
  - [CLI-first (saves tokens vs reading whole files):](#cli-first-saves-tokens-vs-reading-whole-files)
  - [Advanced Token-Saving Enforcement (EXTREME OPTIMIZATION)](#advanced-token-saving-enforcement-extreme-optimization)
  - [Use templates — don't write from scratch:](#use-templates-dont-write-from-scratch)
  - [Use parallel subagents for independent tasks:](#use-parallel-subagents-for-independent-tasks)
  - [One conversation = one task:](#one-conversation-one-task)
  - [GitHub Actions runs tests for free (2000 min/month):](#github-actions-runs-tests-for-free-2000-minmonth)
  - [At the END of every session:](#at-the-end-of-every-session)
- [🧠 Andrej Karpathy's Agent Guidelines](#-andrej-karpathys-agent-guidelines)
...
- [🚫 Never Ask The User Anything That Can Be Inferred](#-never-ask-the-user-anything-that-can-be-inferred)
- [🔧 Autonomous Tool Installation — Agent Handles Everything](#-autonomous-tool-installation-agent-handles-everything)
  - [Installation Method by Tool Type](#installation-method-by-tool-type)
  - [Known Tool → Install Method Map](#known-tool-install-method-map)
  - [Self-Healing Installation Rule](#self-healing-installation-rule)
- [🖼️ README Images — Mandatory Storage Rule](#-readme-images-mandatory-storage-rule)
- [📁 Every New Project — Mandatory Setup](#-every-new-project-mandatory-setup)
  - [Phase 1: Local Setup & Running](#phase-1-local-setup-running)
  - [Phase 2: GitHub Publishing (Only AFTER explicit User approval)](#phase-2-github-publishing-only-after-explicit-user-approval)
- [🎨 UI/UX Standard (Default — Never Needs to Be Asked)](#-uiux-standard-default-never-needs-to-be-asked)
- [🏭 Production Standards (Always Applied — Never Optional)](#-production-standards-always-applied-never-optional)
- [🔧 Self-Healing (Automatic — Never Stops to Ask)](#-self-healing-automatic-never-stops-to-ask)
- [🐙 GitHub Contribution (For External Repos)](#-github-contribution-for-external-repos)
- [🤖 ML/DL Projects — Autonomous Workflow](#-mldl-projects-autonomous-workflow)
  - [Python Environment & Credentials](#python-environment-credentials)
  - [All Configured API Keys (Auto-Loaded)](#all-configured-api-keys-auto-loaded)
  - [Using Each Service in Code](#using-each-service-in-code)
  - [Agent Picks Models Automatically (Transfer Learning First)](#agent-picks-models-automatically-transfer-learning-first)
  - [🧠 MASTER DECISION TREE — Agent Uses This For Every ML Task](#-master-decision-tree-agent-uses-this-for-every-ml-task)
  - [Model Sources — Agent Picks Best Source Per Task](#model-sources-agent-picks-best-source-per-task)
  - [Model Selection by Task](#model-selection-by-task)
  - [Dataset Fetching — Agent Picks Best Source Automatically](#dataset-fetching-agent-picks-best-source-automatically)
  - [VRAM Management (GTX 1650 Ti — Always Apply These)](#vram-management-gtx-1650-ti-always-apply-these)
  - [Experiment Tracking — Weights & Biases (Primary) + MLflow (Local)](#experiment-tracking-weights-biases-primary-mlflow-local)
  - [Data Versioning — DVC](#data-versioning-dvc)
  - [Augmentation Libraries](#augmentation-libraries)
  - [ML Project Structure (Auto-Created by Agent)](#ml-project-structure-auto-created-by-agent)
  - [Kaggle Competition Workflow (Full Automation)](#kaggle-competition-workflow-full-automation)
- [📊 Diagrams, Architecture & Documentation — Mandatory Rules](#-diagrams-architecture-documentation-mandatory-rules)
  - [Core Rule](#core-rule)
  - [Tool Selection — Agent Picks Best Tool Per Diagram Type](#tool-selection-agent-picks-best-tool-per-diagram-type)
  - [DIAGRAM DECISION TREE](#diagram-decision-tree)
  - [Code Templates — Agent Uses These Directly](#code-templates-agent-uses-these-directly)
  - [Matplotlib Visual Quality Standards](#matplotlib-visual-quality-standards)
  - [File Naming Convention](#file-naming-convention)
  - [README Integration Template](#readme-integration-template)
  - [Generate All Diagrams Command](#generate-all-diagrams-command)
- [Current Status: [IN PROGRESS / DONE / BLOCKED]](#current-status-in-progress-done-blocked)
